...
""")
    ...
    if choice == "1":
        ...
        breakpoint()  # Pause before making POST request to create user
        response = requests.post(api_endpoint_users, json=user_data)
        breakpoint()  # Pause after POST request to inspect response
        ...
        breakpoint()  # Pause before GET request to fetch task
        response = requests.get(f"{api_endpoint_tasks}/{user_id}")
        breakpoint()  # Pause after GET request to inspect response
        ...
        breakpoint()  # Pause before POST request to create task
        ...
        breakpoint()  # Pause before PATCH request to update task
        response = requests.patch(f"{api_endpoint_tasks}/{task_id}", json=update_data)
        breakpoint()  # Pause after PATCH request to inspect response
        print("Update response:", response.json())

    elif choice == "8":
        task_id = input("Enter the task ID to delete: ").strip()
        breakpoint()  # Pause before DELETE request to delete task
        response = requests.delete(f"{api_endpoint_tasks}/{task_id}")
        breakpoint()  # Pause after DELETE request to inspect response
